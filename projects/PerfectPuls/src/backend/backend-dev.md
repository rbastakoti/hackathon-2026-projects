...
**Extension Data Processing Pipeline:**
```python
async def process_extension_request(request_data: dict):
    # Extract data from Chrome extension format
    page_content = request_data["page_content"]
    url = request_data["basic_info"]["url"]
    domain = request_data["basic_info"]["domain"]
    
    # Step 1: Parse website content for services and pricing
    services = extract_services_from_text(page_content)
    pricing = extract_pricing_from_text(page_content) 
    provider_info = extract_provider_info(page_content)
    
    # Step 2: Create query embeddings for RAG search
    service_queries = [f"{service} healthcare service" for service in services]
    
    # Step 3: Hybrid RAG search
    rag_results = await hybrid_rag_search(service_queries, top_k=5)
    
    # Step 4: Format response for extension
    return format_extension_response(services, pricing, rag_results)

async def hybrid_rag_search(queries: list, top_k: int = 5):
    combined_results = []
    ...
    for query in queries:
        ...
        // Graph traversal for related coverage and costs
        MATCH (s)<-[:INCLUDES_SERVICE]-(c:Coverage)
        OPTIONAL MATCH (s)-[:HAS_COST]->(cost:Cost)
        OPTIONAL MATCH (c)-[:REQUIRES_PROVIDER]->(p:Provider)
        
        RETURN s, c, cost, p, similarity
        ORDER BY similarity DESC LIMIT $limit
        """
        
        results = await neo4j_session.run(cypher_query, {
...
def format_extension_response(services, pricing, rag_results):
    # Transform RAG results into extension's expected format
    return {
        "summary": generate_summary(services, rag_results),
        "match_checklist": create_checklist_items(rag_results),
        "feasibility": calculate_feasibility_score(rag_results),
        "money_saved": calculate_cost_savings(pricing, rag_results),
        "benefits_services": extract_service_benefits(rag_results),
        "recommendations": generate_recommendations(rag_results)
    }
```
